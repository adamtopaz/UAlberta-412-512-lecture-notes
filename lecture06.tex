...
\begin{proof}
  ...
  The product $\pfrak_{1} \cdots \pfrak_{k} \cdot \qfrak_{1} \cdots \qfrak_{s}$ is then \todo{contained}{Add details.} in $\afrak$.
\end{proof}

\begin{definition}
  For a fractional ideal $\afrak$ of $\Ocal$, we write
  \[ \afrak^{-1} := \{x \in K \ | \ x \cdot \afrak \subset \Ocal\}. \]
  \todo{This is again a fractional ideal.}{Why?}
\end{definition}
...
\end{lemma}
\begin{proof}
  We \todo{may}{Why?} assume that $\pfrak$ is nonzero.
  ...
  \todo{One of the $\pfrak_{i}$ is contained in $\pfrak$, since $\pfrak$ is prime.}{Exercise.}
...
\begin{proof}[Proof of Theorem~\ref{theorem:ideal_factorization}]
  ...
  The maximality of $\afrak$ shows that the \todo{ideal}{Why is this an ideal?} $\afrak \pfrak^{-1}$ admits a factorization as $\pfrak_{1} \cdots \pfrak_{k}$.
...
\begin{proposition}
  ...
  is a surjective morphism of groups whose kernel is the collection of fractional ideals which meet $S$.
\end{proposition}
\begin{proof}
  If $\pfrak$ is a prime which meets $S$, then $S^{-1}\pfrak = (1)$, while the product is compatible with localization:
  \[ S^{-1}(\afrak \bfrak) = S^{-1}\afrak \cdot S^{-1} \bfrak. \]
  If $S^{-1}\afrak = (1)$, then $1 = a/s$ for some $a \in \afrak$ and $s \in S$.
  Thus $s = a$ hence $\afrak$ meets $S$.
...
\begin{proposition}[Killing the class group]
  Suppose that $Cl_{K}(\Ocal)$ is finite, and let $\afrak_{1},\ldots,\afrak_{k}$ be representatives fractinoal ideals of the ideal classes.
